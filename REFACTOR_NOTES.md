...
## 二十、P7 Bug 分析：`workflow_error` 后无 `code_token` 输出

> 本节记录了一次"初步分析错误 → 被用户质疑 → 重新定位 → 正确修复"的完整过程，是 **先证据、再结论** 原则的真实案例。
...
data:{"d":"{\"type\":\"planning_done\",\"detail\":\"retries=2\"}"}
data:{"d":"{\"type\":\"code_gen_start\"}"}
data:{"d":"{\"type\":\"workflow_error\",\"detail\":\"\"}"}   ← detail 为空串

curl: (18) transfer closed with outstanding read data remaining
```

关键特征：
- `code_token` 事件**完全没有**出现
- `detail:""` —— 异常 `getMessage()` 为 null
- `chat_history` 没有新的 `ai_review_report` —— ReviewAgent 根本没跑到

### 20.2 初步（错误）分析：`sink.error(e)` 与 Spring MVC 不兼容
...
修复 `sink.error → sink.complete` 并重启应用后，用户在 22:32:17 再次运行，观察到**输出与修复前完全相同**：

- `code_gen_start` 后立即出现 `workflow_error`，`detail` 仍为空串
- 没有任何 `code_token` 事件
- `chat_history` 仍无新记录

**用户的关键反驳**：`workflow_error` SSE 事件能成功出现，说明 `sink.next()` 执行了，也说明 `sink.error` 不是 **导致 workflow_error 出现** 的原因。初步分析把 `sink.error` 当根因是倒因为果——`sink.error` 的问题只在于它之后 Spring MVC 报错，不是它"制造"了 workflow_error。

**正确推论**：必须找到是什么异常触发了 OrchestratorAgent catch 块，且该异常的 `getMessage()` 返回 null（因为 `detail:""` 说明 `e.getMessage()` 为 null）。

### 20.9 实际根因：`RetryOutputGuardrail` 阻断 streaming path

**字节码分析链**（`~/.m2/repository` 中反汇编 LangChain4j 1.1.0 .class 文件）：

```
AiCodeGeneratorServiceFactory.java — HTML/MULTI_FILE case:
  AiServices.builder(...)
    .streamingChatModel(streamingChatModel)
    .outputGuardrails(new RetryOutputGuardrail())   ← 问题所在
    .build();
```

```
调用路径（streaming）：
TokenStreamToFluxAdapter.adapt()
  → tokenStream.start()                    ← 立即发起 LLM HTTP 请求（非阻塞）
  → LLM 开始流式返回 token

AiServiceStreamingResponseHandler:
  onPartialResponse(token)                  ← token 暂存到 responseBuffer，不发往 sink
  onCompleteResponse(response)
    → executeGuardrails(response)           ← 所有 token 到齐后才跑 guardrail
    → RetryOutputGuardrail.validate(msg)
        → 检查 SENSITIVE_WORDS: {"password","secret","token","api key","api_key","credential"}
        → HTML 代码几乎必然包含 "token"（CSS variable token、JS event token…）
        → reprompt() 触发 → 重试
        → 重试耗尽 → OutputGuardrailException 或 NPE（aiMessage 为 null）
        → e.getMessage() == null
    → 异常从 onCompleteResponse() 抛出
    → tokens 永远不被 replay 到 Sinks → Flux 永远不完成也不报错
    → 但异常通过 errorHandler 路径到达 sinks.tryEmitError()
    → blockLast() 抛 ReactorException，包装原始异常（message 为 null）
    → OrchestratorAgent 外层 catch 捕获：e.getMessage() == null → detail:""
```

**为什么 `RetryOutputGuardrail` 绝对不能用在 streaming 路径上**：

| 对比点 | 非流式（batch） | 流式（streaming） |
|--------|----------------|-------------------|
| token 何时到达 sink | AiServices 同步返回后 | 应该实时 emit |
| guardrail 执行时机 | 同步调用链中，阻塞直到结果 | onCompleteResponse 里，所有 token 已缓冲 |
| guardrail 失败后果 | 抛异常，调用方正常捕获 | tokens 不 replay，Flux 挂起或 emit error with null message |
| SENSITIVE_WORDS "token" | 非常规检查词，不常出现 | HTML/JS 必然出现（CSS token、event token、auth token…） |

RetryOutputGuardrail 的 Javadoc 也明确写了："only use output guardrails in non-streaming (batch) contexts"。

### 20.10 修复：移除 `RetryOutputGuardrail`

**改动文件**：`AiCodeGeneratorServiceFactory.java`
...
yield AiServices.builder(AiCodeGeneratorService.class)
        ...
        .outputGuardrails(new RetryOutputGuardrail())   // ← 移除
        .build();
...
yield AiServices.builder(AiCodeGeneratorService.class)
        ...
        .chatMemory(chatMemory)
        .inputGuardrails(new PromptSafetyInputGuardrail())
        // RetryOutputGuardrail 已移除：SENSITIVE_WORDS 含 "token"，
        // HTML 代码必然触发 reprompt → null message exception → detail:""
        // Output guardrail 只能用于非流式（batch）场景
        .build();
```

`RetryOutputGuardrail` import 同步删除（避免 unused import 警告）。

**11 个单元测试全部通过**（`OrchestratorAgentTest` 10 个 + `ReviewModelBeanCapabilityTest` 1 个）。

### 20.11 两个修复的关系

| 修复 | 改动 | 必要性 | 说明 |
|------|------|--------|------|
| `sink.error → sink.complete` | OrchestratorAgent.java | 是（防御性） | 即使根因修复后，未来其他异常仍可能走到 catch；`sink.error` 在 Spring MVC SSE 上有副作用 |
| 移除 `RetryOutputGuardrail` | AiCodeGeneratorServiceFactory.java | 是（根因修复） | 直接消除 null-message exception 的来源；让 code_token 事件能正常流出 |

两个修复都保留，互相独立，各有意义。

### 20.12 总结教训

1. **先证据，再结论**：看到 `workflow_error` 出现，应该先问"什么异常触发了 catch 块，且 getMessage() 为 null"，而不是先猜测"sink.error 引发了 workflow_error"。
2. **null message 是诊断关键**：`detail:""` 告诉你 `e.getMessage() == null`，这是定位 guardrail NPE 路径的直接线索。
3. **Guardrail 在流式场景是隐形地雷**：不报错，只是 tokens 不来，Flux 挂起，或者 exception 的 message 为 null 导致 detail 为空。关键诊断日志：`log.error("..., exceptionClass={}, message={}", e.getClass().getName(), e.getMessage(), e)` —— 加到外层 catch，生产可用，已保留（去掉 DEBUG-P7 临时前缀）。
4. **doOnNext debug 日志要删**：每个 token 都打一行日志，在流式路径上是性能杀手，验证完成后立即删除。
...
**Q: P7 联调时遇到什么印象深刻的 bug？**

A: 一个"output guardrail 在 streaming 路径上静默失败"的 bug。现象是：SSE 流到 `code_gen_start` 后立刻出现 `workflow_error`，没有任何 `code_token`，错误 detail 是空串。

我犯了一个错误：第一次把原因归结为 `sink.error()` 触发了 Spring MVC 的 `LinkedHashMap` 序列化问题，修了，重跑，输出一模一样——被用户当场质疑。被迫回到证据优先：`detail:""` 说明 `e.getMessage() == null`，这是定位方向。

字节码分析（javap 反汇编 LangChain4j 1.1.0 .class）发现：`AiServiceStreamingResponseHandler.onCompleteResponse()` 在所有 token 到齐后才跑 output guardrail，此时 token 已缓冲但尚未 replay 到 Flux。`RetryOutputGuardrail.SENSITIVE_WORDS` 包含 `"token"` 这个词——而 HTML/JS 代码几乎必然出现 "token"（CSS token、event token、auth token）。guardrail 触发 reprompt，重试耗尽后抛出 null-message exception，tokens 永远不 replay，Flux 挂起或抛 wrapped null-message exception 到外层 catch。

修复一行：移除 `RetryOutputGuardrail`。真实环境跑通，score=95，code_token 事件流正常输出，workflow_done 正常结束。

教训：**output guardrail 只适合非流式（batch）场景**。LangChain4j 的 Javadoc 里写着，但当时没有认真读。
...
- 修复 4 个 bug（chat_history 写入缺失 / 线程池泄漏 / Guardrail 字符限制 / streaming 路径 output guardrail 静默失败）
...
- v1.0 - P1-P4 笔记（初版）
- v2.0 - P5-P7 补全 + 配置规范化 + 面试讲点
- v2.1 - P7 Bug 复盘（二十、streaming guardrail 根因分析 + 修复）+ 面试讲点 21.2 补充 P7 bug Q&A
- 状态：Multi-Agent 重构 100% 完成，真实环境验证通过（2026-05-25）
  - score=95，code_token 事件流正常，workflow_done 正常，chat_history ai_review_report 写入正常
